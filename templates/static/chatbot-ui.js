...
function init() {
    ...
    //--------------------------- Important Variables----------------------------
    botLogoPath = "static/bot-logo.png"

    //--------------------------- Chatbot Frontend -------------------------------
    const chatContainer = document.getElementById("chat-container");

    template = ` <button class='chat-btn'><img src = "static/comment.png" class = "icon" ></button>
    ...
    <div class='chat-popup'>
		...
		<div class='chat-header'>
			...
			<h3 class='bot-title'>업무지원 ChatBot</h3>
			<button class = "expand-chat-window" ><img src="static/open_fullscreen.png" class="icon" ></button>
		</div>
    ...
    //--------------------------- Important Variables----------------------------
    var inactiveMessage = "지금은 이용할 수 없습니다. 잠시후에 다시 시도해 주세요."
    ...
    chatBtn.addEventListener("click", () => {
        ...
        if (chatPopup.style.display == "none" && mobileDevice) {
            ...
            chatInput.focus();
            chatBtn.innerHTML = `<img src = "static/close.png" class = "icon" >`
        } else if (mobileDevice) {
            chatPopup.style.display = "none"
            chatBtn.innerHTML = `<img src = "static/comment.png" class = "icon" >`
        } else {
            mobileView()
        }
    })
    ...
    expandWindow.addEventListener("click", (e) => {
        ...
        if (expandWindow.innerHTML == '<img src="static/open_fullscreen.png" class="icon">') {
            expandWindow.innerHTML = `<img src = "static/close_fullscreen.png" class = 'icon'>`
            root.style.setProperty('--chat-window-height', 80 + "%");
            root.style.setProperty('--chat-window-total-width', 85 + "%");
        } else if (expandWindow.innerHTML == '<img src="static/close.png" class="icon">') {
            chatPopup.style.display = "none"
            chatBtn.style.display = "block"
        } else {
            expandWindow.innerHTML = `<img src = "static/open_fullscreen.png" class = "icon" >`
...
function mobileView() {
    ...
    if (chatPopup.style.display == "none") {
        ...
        expandWindow.innerHTML = `<img src = "static/close.png" class = "icon" >`
    }
}
